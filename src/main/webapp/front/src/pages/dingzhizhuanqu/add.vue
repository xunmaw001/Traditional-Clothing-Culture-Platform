...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="定制编号" prop="dingzhibianhao">
              <el-input v-model="ruleForm.dingzhibianhao" placeholder="定制编号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="定制标题" prop="dingzhibiaoti">
            <el-input v-model="ruleForm.dingzhibiaoti" 
                placeholder="定制标题" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="定制类别" prop="dingzhileibie">
            <el-select v-model="ruleForm.dingzhileibie" placeholder="请选择定制类别" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="定制样板" v-if="type!='cross' || (type=='cross' && !ro.dingzhiyangban)" prop="dingzhiyangban">
            <file-upload
            tip="点击上传定制样板"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="定制样板" prop="dingzhiyangban">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dingzhiyangban.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="定制价格" prop="dingzhijiage">
            <el-input v-model="ruleForm.dingzhijiage" 
                placeholder="定制价格" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="定制详情" prop="dingzhixiangqing">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='dingzhiyangban'){
              this.ruleForm.dingzhiyangban = obj[o].split(",")[0];
        ...
        this.$http.get('option/dingzhileibie/dingzhileibie', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('dingzhizhuanqu/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('dingzhizhuanqu/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('dingzhizhuanqu/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      dingzhiyangbanUploadChange(fileUrls) {
          this.ruleForm.dingzhiyangban = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
