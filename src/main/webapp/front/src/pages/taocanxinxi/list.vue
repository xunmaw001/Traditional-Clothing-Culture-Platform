...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>套餐标题</div>
        <el-input v-model="formSearch.taocanbiaoti" placeholder="套餐标题" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>套餐类别</div>
        <el-select v-model="formSearch.taocanleibie" placeholder="请选择套餐类别" :clearable="true">
          <el-option v-for="(item, index) in taocanleibieOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>款式</div>
        <el-select v-model="formSearch.kuanshi" placeholder="请选择款式" :clearable="true">
          <el-option v-for="(item, index) in kuanshiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('taocanxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(255,215,169,1) 0%, rgba(255,164,58,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/taocanxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 20px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"10px 10px","borderColor":"#52b646","borderRadius":"8px","flexWrap":"wrap","background":"#fff","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid double solid double","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"2px solid #88d67e","cursor":"pointer","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","margin":"10px","borderRadius":"8px","background":"#fff","display":"inline-block","width":"23%","position":"relative","borderStyle":"dotted dashed solid double","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","objectFit":"cover","borderRadius":"8px","display":"block","height":"275px"}' v-if="item.taocanzhaopian && item.taocanzhaopian.substr(0,4)=='http'" :src="item.taocanzhaopian" class="image" />
				<img :style='{"width":"100%","objectFit":"cover","borderRadius":"8px","display":"block","height":"275px"}' v-else :src="baseUrl + (item.taocanzhaopian?item.taocanzhaopian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">套餐标题:{{item.taocanbiaoti}}</div>
			</div>
		</div>
	...
	<!-- 热门信息 -->
	<div class="hot" :style='{"width":"100%","margin":"20px 0 20px","background":"none","height":"auto"}'>
	  <div :style='{"padding":"0 20px","borderColor":"#52b646","color":"#52b646","textAlign":"center","background":"#e6f5e5","borderWidth":"3px","width":"100%","lineHeight":"44px","fontSize":"14px","borderStyle":"solid double dashed double"}'>热门信息</div>
	  <div :style='{"padding":"20px","boxShadow":"0 1px 2px rgba(0,0,0,.1)","borderRadius":"0 0 12px 12px","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item" :style='{"cursor":"pointer","border":"2px solid #88d67e","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","borderRadius":"8px","background":"none","width":"23%","borderStyle":"dotted dashed solid double","height":"auto"}' @click="toDetail(item)">
	      <img :style='{"width":"100%","objectFit":"cover","borderRadius":"8px","display":"block","height":"180px"}' :src="baseUrl + (item.taocanzhaopian?item.taocanzhaopian.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"16px","color":"#333"}'>{{item.taocanbiaoti}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/taocanleibie/taocanleibie').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.taocanbiaoti = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.taocanbiaoti != '') searchWhere.taocanbiaoti = '%' + this.formSearch.taocanbiaoti + '%';
        if (this.formSearch.taocanleibie != '') searchWhere.taocanleibie = this.formSearch.taocanleibie;
        if (this.formSearch.kuanshi != '') searchWhere.kuanshi = this.formSearch.kuanshi;
        if (this.curFenlei != '全部') searchWhere.taocanleibie = this.curFenlei;
        this.$http.get('taocanxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/taocanxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
