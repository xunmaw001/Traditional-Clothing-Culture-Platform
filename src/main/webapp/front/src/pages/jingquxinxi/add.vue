...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点编号" prop="jingdianbianhao">
              <el-input v-model="ruleForm.jingdianbianhao" placeholder="景点编号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点标题" prop="jingdianbiaoti">
            <el-input v-model="ruleForm.jingdianbiaoti" 
                placeholder="景点标题" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点照片" v-if="type!='cross' || (type=='cross' && !ro.jingdianzhaopian)" prop="jingdianzhaopian">
            <file-upload
            tip="点击上传景点照片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="景点照片" prop="jingdianzhaopian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jingdianzhaopian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点位置" prop="jingdianweizhi">
            <el-input v-model="ruleForm.jingdianweizhi" 
                placeholder="景点位置" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点简介" prop="jingdianjianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="景点简介"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="景点详情" prop="jingdianxiangqing">
            <editor 
...
<script>
  export default {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jingdianzhaopian'){
              this.ruleForm.jingdianzhaopian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jingquxinxi/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('jingquxinxi/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('jingquxinxi/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      jingdianzhaopianUploadChange(fileUrls) {
          this.ruleForm.jingdianzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
